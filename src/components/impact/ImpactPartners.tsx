...
const PartnerTestimonial = ({ quote, authorTitle, company, highlighted, delay }: PartnerTestimonialProps) => (
  <motion.div
    ...
    className={`p-5 md:p-8 rounded-[24px] flex flex-col justify-between min-h-[220px] sm:min-h-[260px] md:min-h-[320px] transition-all duration-300 ${
      highlighted 
  ...
  >
    <div>
      <Quote size={28} className="text-[#A64AED]/30 mb-3 md:mb-4" fill="currentColor" />
      <p className="text-[#1A1A1A] text-[15px] md:text-[16px] font-medium leading-relaxed mb-4 md:mb-6">
        "{quote}"
      </p>
    </div>

    <div className="flex items-center gap-4 pt-4 md:pt-5 border-t border-[#1A1A1A]/5">
      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold overflow-hidden shrink-0 ${
      ...
      <div>
        <h4 className="text-[14px] md:text-[15px] font-bold text-[#1A1A1A] leading-tight">{authorTitle}</h4>
        <p className="text-[#6B7280] text-[12px] md:text-[13px] font-medium">
          {company}
        </p>
      </div>
    </div>
...
const ImpactPartners = () => {
  return (
    <section className="bg-[#FDFDFD] py-12 md:py-20 overflow-hidden">
      <div className="max-w-[1280px] mx-auto px-6 lg:px-12">
        <motion.h2 
          initial={{ opacity: 0, y: -20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="text-[26px] sm:text-[36px] md:text-[48px] font-bold text-[#1A1A1A] text-center mb-10 md:mb-12 relative"
        >
          What Our Partners Say
          <span className="absolute -bottom-3 left-1/2 -translate-x-1/2 w-16 h-1 bg-[#A64AED] rounded-full opacity-30"></span>
        </motion.h2>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 items-stretch max-w-[1184px] mx-auto">
          <PartnerTestimonial 
